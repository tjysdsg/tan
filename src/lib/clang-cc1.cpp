...
static int PrintSupportedCPUs(std::string TargetStr) {
  ...
  std::unique_ptr<llvm::TargetMachine>
      TheTargetMachine(TheTarget->createTargetMachine(TargetStr, "", "+cpuHelp", Options, None));
  return 0;
}
...
int cc1_main(ArrayRef<const char *> Argv, const char *Argv0, void *MainAddr) {
  ...
  if (llvm::timeTraceProfilerEnabled()) {
    ...
    auto profilerOutput = Clang->createOutputFile(Path.str(),
        /*Binary=*/false,
    ...
    llvm::errs() << "Use chrome://tracing or Speedscope App "
