...
namespace {
enum class Format { Default, GNU, BSD, DARWIN, Unknown };
}

static Format FormatType = Format::Default;
...
static object::Archive::Kind getKindFromMember(const NewArchiveMember &Member) {
  Expected<std::unique_ptr<object::ObjectFile>>
      OptionalObject = object::ObjectFile::createObjectFile(Member.Buf->getMemBufferRef());
  ...
  for (size_t i = 1; i < Argv.size(); ++i) {
    ...
    if (Arg[0] == '-') {
      ...
      if (Arg == "M") {
        ...
        FormatType = StringSwitch<Format>(match).Case("default", Format::Default)
            .Case("gnu", Format::GNU)
            .Case("darwin", Format::DARWIN)
            .Case("bsd", Format::BSD)
            .Default(Format::Unknown);
        if (FormatType == Format::Unknown) {
