...
static object::Archive &readLibrary(const Twine &Library) {
  ...
  failIfError(errorToErrorCode(LibOrErr.takeError()), "could not parse library");
...
static std::string normalizePath(StringRef Path) {
  return CompareFullPath ? sys::path::convert_to_slash(Path) : std::string(sys::path::filename(Path));
}
...
static void doExtract(StringRef Name, const object::Archive::Child &C) {
  ...
  int FD;
  failIfError(sys::fs::openFileForWrite(outputFilePath, FD, sys::fs::CD_CreateAlways, sys::fs::OF_None, Mode), Name);
  ...
  if (OriginalDates) {
    auto ModTimeOrErr = C.getLastModified();
    failIfError(ModTimeOrErr.takeError());
    failIfError(sys::fs::setLastAccessAndModificationTime(FD, ModTimeOrErr.get()));
  }
...
static void performReadOperation(ArchiveOperation Operation, object::Archive *OldArchive) {
...
static void addChildMember(std::vector<NewArchiveMember> &Members,
  ...
  if (Thin && FlattenArchive) {
    ...
    if (sys::path::is_absolute(*FileNameOrErr)) {
      ...
      failIfError(FileNameOrErr.takeError());
      Expected<std::string> PathOrErr = computeArchiveRelativePath(ArchiveName, *FileNameOrErr);
      NMOrErr->MemberName = Saver.save(PathOrErr ? *PathOrErr : sys::path::convert_to_slash(*FileNameOrErr));
    }
  }
  if (FlattenArchive && identify_magic(NMOrErr->Buf->getBuffer()) == file_magic::archive) {
...
static void addMember(std::vector<NewArchiveMember> &Members, StringRef FileName, bool FlattenArchive = false) {
  ...
  if (!Thin) {
    ...
    if (sys::path::is_absolute(FileName)) {
      ...
      Expected<std::string> PathOrErr = computeArchiveRelativePath(ArchiveName, FileName);
      NMOrErr->MemberName = Saver.save(PathOrErr ? *PathOrErr : sys::path::convert_to_slash(FileName));
    }
  }
...
enum InsertAction {
  IA_AddOldMember, IA_AddNewMember, IA_Delete, IA_MoveOldMember, IA_MoveNewMember
};
...
static std::vector<NewArchiveMember> computeNewArchiveMembers(ArchiveOperation Operation, object::Archive *OldArchive) {
  ...
  if (OldArchive) {
    ...
    for (auto &Child : OldArchive->children(Err)) {
      ...
      InsertAction Action = computeInsertAction(Operation, Child, Name, MemberI, MemberCount);
      switch (Action) {
...
static void performWriteOperation(ArchiveOperation Operation,
  ...
  switch (FormatType) {
    case Default:
      if (Thin) {
        ...
        Kind = !NewMembersP->empty() ? getKindFromMember(NewMembersP->front()) : getDefaultForHost();
      } else {
  ...
  Error E = writeArchive(ArchiveName,
      NewMembersP ? *NewMembersP : NewMembers,
      Symtab,
      Kind,
      Deterministic,
      Thin,
...
static void performOperation(ArchiveOperation Operation,
  ...
  switch (Operation) {
    ...
    case QuickAppend:
    case ReplaceOrInsert:
      performWriteOperation(Operation, OldArchive, std::move(OldArchiveBuf), NewMembers);
      return;
    case CreateSymTab:
...
static int performOperation(ArchiveOperation Operation, std::vector<NewArchiveMember> *NewMembers) {
  // Create or open the archive object.
  ErrorOr<std::unique_ptr<MemoryBuffer>> Buf = MemoryBuffer::getFile(ArchiveName, -1, false);
...
static void runMRIScript() {
  ...
  for (line_iterator I(Ref, /*SkipBlanks*/ false), E; I != E; ++I) {
    ...
    auto Command = StringSwitch<MRICommand>(CommandStr.lower()).Case("addlib", MRICommand::AddLib)
...
static cl::TokenizerCallback getRspQuoting(ArrayRef<const char *> ArgsArr) {
  cl::TokenizerCallback Ret = Triple(sys::getProcessTriple()).getOS() == Triple::Win32 ? cl::TokenizeWindowsCommandLine
                                                                                       : cl::TokenizeGNUCommandLine;

  for (ArrayRef<const char *>::iterator ArgIt = ArgsArr.begin(); ArgIt != ArgsArr.end(); ++ArgIt) {
...
static int ar_main(int argc, char **argv) {
  ...
  for (ArrayRef<const char *>::iterator ArgIt = Argv.begin(); ArgIt != Argv.end(); ++ArgIt) {
    ...
    if (Match) {
      FormatType = StringSwitch<Format>(Match).Case("default", Default)
          .Case("gnu", GNU)
          .Case("darwin", DARWIN)
          .Case("bsd", BSD)
          .Default(Unknown);
    ...
    if (matchFlagWithArg("plugin", ArgIt, Argv) || matchFlagWithArg("rsp-quoting", ArgIt, Argv)) {
      continue;
    }
...
extern int llvm_ar_main(int argc, char **argv) {
  ...
  auto Is = [](StringRef Tool) {
    ...
    return I != StringRef::npos && (I + Tool.size() == Stem.size() || !isAlnum(Stem[I + Tool.size()]));
  };
