...
static std::unique_ptr<raw_fd_ostream> getOutputStream(StringRef Path, DiagnosticsEngine &Diags, bool Binary) {
  ...
  std::error_code EC;
  auto Out = std::make_unique<raw_fd_ostream>(Path, EC, (Binary ? sys::fs::OF_None : sys::fs::OF_Text));
  if (EC) {
...
static bool ExecuteAssembler(AssemblerInvocation &Opts, DiagnosticsEngine &Diags) {
  ...
  if (Opts.OutputType == AssemblerInvocation::FT_Asm) {
    MCInstPrinter
        *IP = TheTarget->createMCInstPrinter(llvm::Triple(Opts.Triple), Opts.OutputAsmVariant, *MAI, *MCII, *MRI);
    ...
    std::unique_ptr<MCObjectWriter>
        OW = DwoOS ? MAB->createDwoObjectWriter(*Out, *DwoOS) : MAB->createObjectWriter(*Out);
  ...
  if (Opts.EmbedBitcode && Ctx.getObjectFileInfo()->getObjectFileType() == MCObjectFileInfo::IsMachO) {
...
int cc1as_main(ArrayRef<const char *> Argv, const char *Argv0, void *MainAddr) {
  ...
  if (Asm.ShowHelp) {
    getDriverOptTable().PrintHelp(llvm::outs(), "clang -cc1as [options] file...", "Clang Integrated Assembler",
