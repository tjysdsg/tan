...
void AnalyzerImpl::analyze_struct(const ASTBasePtr &p) {
  ...
  for (size_t i = 0; i < n; ++i) {
    ...
    if (m->get_node_type() == ASTNodeType::VAR_DECL) { /// member variable without initial value
      ...
      ty->append_child(_h.get_ty(init_val)); /// init_val->_type->_default_value is set to the initial value
    } else {
