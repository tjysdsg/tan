...
Expr::Expr(ASTNodeType type, TokenizedSourceFile *src, int bp) : ASTBase(type, src, bp) {}

vector<ASTBase *> Expr::get_children() const { return {}; }

/// \section Literals

Literal::Literal(ASTNodeType type, TokenizedSourceFile *src, int bp) : Expr(type, src, bp) {}

BoolLiteral *BoolLiteral::Create(TokenizedSourceFile *src, bool val) {
...
BoolLiteral::BoolLiteral(TokenizedSourceFile *src) : Literal(ASTNodeType::BOOL_LITERAL, src, 0) {}

IntegerLiteral::IntegerLiteral(TokenizedSourceFile *src) : Literal(ASTNodeType::INTEGER_LITERAL, src, 0) {}

IntegerLiteral *IntegerLiteral::Create(TokenizedSourceFile *src, uint64_t val, bool is_unsigned) {
...
FloatLiteral *FloatLiteral::Create(TokenizedSourceFile *src, double val) {
...
FloatLiteral::FloatLiteral(TokenizedSourceFile *src) : Literal(ASTNodeType::FLOAT_LITERAL, src, 0) {}

StringLiteral *StringLiteral::Create(TokenizedSourceFile *src, const str &val) {
...
StringLiteral::StringLiteral(TokenizedSourceFile *src) : Literal(ASTNodeType::STRING_LITERAL, src, 0) {}

CharLiteral *CharLiteral::Create(TokenizedSourceFile *src, uint8_t val) {
...
CharLiteral::CharLiteral(TokenizedSourceFile *src) : Literal(ASTNodeType::CHAR_LITERAL, src, 0) {}

ArrayLiteral *ArrayLiteral::Create(TokenizedSourceFile *src, vector<Literal *> val) {
  auto ret = new ArrayLiteral(src);
  ret->_elements = std::move(val);
  return ret;
}

ArrayLiteral *ArrayLiteral::Create(TokenizedSourceFile *src) { return new ArrayLiteral(src); }
...
ArrayLiteral::ArrayLiteral(TokenizedSourceFile *src) : Literal(ASTNodeType::ARRAY_LITERAL, src, 0) {}

NullPointerLiteral::NullPointerLiteral(TokenizedSourceFile *src) : Literal(ASTNodeType::NULLPTR_LITERAL, src, 0) {}

NullPointerLiteral *NullPointerLiteral::Create(TokenizedSourceFile *src) { return new NullPointerLiteral(src); }

/// \section Identifier

VarRef *VarRef::Create(TokenizedSourceFile *src, const str &name, Decl *referred) {
...
VarRef::VarRef(TokenizedSourceFile *src) : Expr(ASTNodeType::VAR_REF, src, 0) { _is_lvalue = true; }
...
Identifier::Identifier(TokenizedSourceFile *src) : Expr(ASTNodeType::ID, src, 0) {}

Identifier *Identifier::Create(TokenizedSourceFile *src, const str &name) {
...
BinaryOperator::BinaryOperator(BinaryOpKind op, TokenizedSourceFile *src)
...
BinaryOperator *BinaryOperator::Create(BinaryOpKind op, TokenizedSourceFile *src) {
  return new BinaryOperator(op, src);
}

BinaryOperator *BinaryOperator::Create(BinaryOpKind op, TokenizedSourceFile *src, Expr *lhs, Expr *rhs) {
...
UnaryOperator::UnaryOperator(UnaryOpKind op, TokenizedSourceFile *src)
    : Expr(ASTNodeType::UOP, src, UnaryOperator::UOPPrecedence[op]), _op(op) {}

void UnaryOperator::set_rhs(Expr *rhs) { _rhs = rhs; }

UnaryOperator *UnaryOperator::Create(UnaryOpKind op, TokenizedSourceFile *src) { return new UnaryOperator(op, src); }

UnaryOperator *UnaryOperator::Create(UnaryOpKind op, TokenizedSourceFile *src, Expr *rhs) {
...
Parenthesis *Parenthesis::Create(TokenizedSourceFile *src) { return new Parenthesis(src); }

Parenthesis::Parenthesis(TokenizedSourceFile *src)
...
MemberAccess *MemberAccess::Create(TokenizedSourceFile *src) { return new MemberAccess(src); }

MemberAccess::MemberAccess(TokenizedSourceFile *src) : BinaryOperator(BinaryOpKind::MEMBER_ACCESS, src) {}
...
FunctionCall *FunctionCall::Create(TokenizedSourceFile *src) { return new FunctionCall(src); }

FunctionCall::FunctionCall(TokenizedSourceFile *src) : Expr(ASTNodeType::FUNC_CALL, src, PREC_LOWEST) {}
...
Assignment *Assignment::Create(TokenizedSourceFile *src) { return new Assignment(src); }

Assignment::Assignment(TokenizedSourceFile *src)
    : Expr(ASTNodeType::ASSIGN, src, ASTBase::OpPrecedence[ASTNodeType::ASSIGN]) {}
...
Cast *Cast::Create(TokenizedSourceFile *src) { return new Cast(src); }

Cast::Cast(TokenizedSourceFile *src) : Expr(ASTNodeType::CAST, src, ASTBase::OpPrecedence[ASTNodeType::CAST]) {}

vector<ASTBase *> Cast::get_children() const { return {_lhs}; }

bool Cast::is_comptime_known() { return _lhs->is_comptime_known(); }

BinaryOrUnary::BinaryOrUnary(TokenizedSourceFile *src, int bp) : Expr(ASTNodeType::BOP_OR_UOP, src, bp) {}

BinaryOrUnary *BinaryOrUnary::Create(TokenizedSourceFile *src, int bp) { return new BinaryOrUnary(src, bp); }
...
BoolLiteral *Literal::CreateBoolLiteral(TokenizedSourceFile *src, bool val) {
  auto *ret = BoolLiteral::Create(src, val);
  ret->set_type(Type::GetBoolType());
  return ret;
}

FloatLiteral *Literal::CreateFloatLiteral(TokenizedSourceFile *src, double val, size_t bit_size) {
  auto *ret = FloatLiteral::Create(src, val);
  ret->set_type(Type::GetFloatType(bit_size));
  return ret;
}

StringLiteral *Literal::CreateStringLiteral(TokenizedSourceFile *src, str val) {
  auto *ret = StringLiteral::Create(src, val);
  ret->set_type(Type::GetStringType());
  return ret;
}

CharLiteral *Literal::CreateCharLiteral(TokenizedSourceFile *src, uint8_t val) {
  auto *ret = CharLiteral::Create(src, val);
  ret->set_type(Type::GetCharType());
  return ret;
}

ArrayLiteral *Literal::CreateArrayLiteral(TokenizedSourceFile *src, Type *element_type, vector<Literal *> elements) {
...
NullPointerLiteral *Literal::CreateNullPointerLiteral(TokenizedSourceFile *src, Type *element_type) {
