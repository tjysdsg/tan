    ...
    if (curr + 1 < max_len) {                             // hex or binary
      if (str[curr + 1] == 'x' || str[curr + 1] == 'X') { // hex
        ...
        if (curr - start == 2) // only "0x", then still dec
          goto dec;
      } else if (str[curr + 1] == 'b' || str[curr + 1] == 'B') { // binary
        curr += 2;
        char c = str[curr];
...
ret:
  ...
  t->type = INT;
  t->val = val;
  assert(start == current); // we must not change current's value until now
  current = current - 1;
  return t;
}
...
void Lexer::lex() {
  ...
  while (!line.empty()) {
    ...
    while (true) {
      _token_infos.reserve(sizeof(token_info *));
      if (current >= line_len) {
        break;
        ...
        if (current >= line_len)
          break; // EOF
      } else if (std::isalpha(line[current]) || line[current] == '_') {
      } else {
      }
    }
...
#ifdef DEBUG_ENABLED
void Lexer::read_string(const std::string &code) { _reader->read_string(code); }
