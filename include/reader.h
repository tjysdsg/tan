...
namespace tanlang {
	...
	struct line_info {
		...
		line_info() = default;
		explicit line_info(const unsigned lineno) {
			this->lineno = lineno;
		}
		~line_info() = default;
	};
	...
	public:
		...
		char operator[](const code_ptr &ptr) const {
			...
			return _lines[static_cast<size_t>(ptr.r)]
					->code[static_cast<size_t>(ptr.c)];
		}
		// start inclusive, end exclusive string slice
		std::string operator()(const code_ptr &start,
		                       code_ptr end = code_ptr::npos()) const {
			...
			if (end.r == -1 && end.c == -1) {
				...
				end.c = static_cast<long>(
						_lines[static_cast<size_t>(end.r)]->code.length());
			}
			auto s_row = start.r;
			auto e_row = end.r;
			std::string ret;
			if (s_row == e_row) {
				...
				ret = _lines[static_cast<size_t>(s_row)]->code.substr(
						static_cast<unsigned long>(start.c),
				...
				ret += _lines[static_cast<size_t>(e_row)]->code.substr(
						0, static_cast<size_t>(end.c));
			}
			return ret;
		}
		[[nodiscard]] code_ptr front_ptr() const {
			return code_ptr(0, 0);
		}
		[[nodiscard]] code_ptr back_ptr() const {
			return code_ptr(
					static_cast<long>(_lines.size() - 1),
					static_cast<long>(_lines.back()->code.length() - 1));
		}
		// return a copy of code_ptr that points to the next position of ptr
		[[nodiscard]] code_ptr forward_ptr(code_ptr ptr) {
			long n_cols = static_cast<long>(
					_lines[static_cast<size_t>(ptr.r)]->code.length());
			if (ptr.c == n_cols - 1) {
